...
def upload_to_vk(video_path, title="", description=""):
    """
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🏛️ 고대 여성들의 역사!\n\n#여성역사 #고대역사 #교육 #문화"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python upload_vk.py <video_path> [title] [description]")
        sys.exit(1)
    
    video_path = sys.argv[1]
    title = sys.argv[2] if len(sys.argv) > 2 else "고대 여성들의 역사"
    description = sys.argv[3] if len(sys.argv) > 3 else "🏛️ 고대 여성들의 역사!"
    
    try:
        result = upload_to_vk(video_path, title, description)
        print("\n✅ Success!")
